...
uint8_t Secp256K1::GetByte(std::string &str, int idx) {
  ...
  if (sscanf(tmp, "%X", &val) != 1) {
    printf(
        "ParsePublicKeyHex: Error invalid public key specified (unexpected "
        "hexadecimal digit)\n");
    exit(-1);
  }
...
bool Secp256K1::ParsePublicKeyHex(std::string str, Point &ret,
                                  bool &isCompressed) {
  ret.Clear();

  if (str.length() < 2) {
    printf(
        "ParsePublicKeyHex: Error invalid public key specified (66 or 130 "
        "character length)\n");
    return false;
  }
  ...
  switch (type) {
    case 0x02:
      if (str.length() != 66) {
        printf(
            "ParsePublicKeyHex: Error invalid public key specified (66 "
            "character length)\n");
        return false;
      }
      for (int i = 0; i < 32; i++) ret.x.SetByte(31 - i, GetByte(str, i + 1));
    ...
    case 0x04:
      if (str.length() != 130) {
        printf(
            "ParsePublicKeyHex: Error invalid public key specified (130 "
            "character length)\n");
        exit(-1);
      }
      for (int i = 0; i < 32; i++) ret.x.SetByte(31 - i, GetByte(str, i + 1));
      for (int i = 0; i < 32; i++) ret.y.SetByte(31 - i, GetByte(str, i + 33));
      isCompressed = false;
      break;
    ...
    default:
      printf(
          "ParsePublicKeyHex: Error invalid public key specified (Unexpected "
          "prefix (only 02,03 or 04 allowed)\n");
      return false;
  }
  ...
  if (!EC(ret)) {
    printf(
        "ParsePublicKeyHex: Error invalid public key specified (Not lie on "
        "elliptic curve)\n");
    return false;
  }
...
std::vector<Point> Secp256K1::AddDirect(std::vector<Point> &p1,
                                        std::vector<Point> &p2) {
  if (p1.size() != p2.size()) {
    // Fatal error
...
Point Secp256K1::AddPoints(Point &a, Point &b) {
    ...
    if (a.equals(b)) return Double(a);

    if (a.x.IsEqual(&b.x) && !a.y.IsEqual(&b.y)) return Point(); // Punto en el infinito
...
std::vector<Point> Secp256K1::GeneratePoints(Int &start, Int &end) {
    std::vector<Point> points;
    for (Int i = start; i.IsLowerOrEqual(&end); i.AddOne()) {
        points.push_back(ComputePublicKey(&i));
    }
    return points;
}

Point Secp256K1::Subtract(Point &a, Point &b) {
    Point b_neg = b;
    b_neg.y.ModNeg();
    return AddPoints(a, b_neg);
}

std::vector<Point> Secp256K1::AddMultiple(Point &a, Point &b, int n) {
    std::vector<Point> results;
    results.reserve(n);
    Point current = a;
    for (int i = 0; i < n; i++) {
        current = AddPoints(current, b);
        results.push_back(current);
    }
    return results;
}

std::vector<Point> Secp256K1::SubtractMultiple(Point &a, Point &b, int n) {
    std::vector<Point> results;
    results.reserve(n);
    Point current = a;
    Point b_neg = b;
    b_neg.y.ModNeg();
    for (int i = 0; i < n; i++) {
        current = AddPoints(current, b_neg);
        results.push_back(current);
    }
    return results;
}
